...
use common\widgets\icon_upload\Uploader;
...
class SettingsController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow'   => true,
                        'actions' => ['profile', 'account', 'networks', 'disconnect','upload'],
                        'roles'   => ['@'],
                    ],
                    [
                        'allow'   => true,
    ...
    public function actionProfile()
    {
        ...
        if(Yii::$app->request->isPost){
            ...
            //$i = Yii::$app->request->post('Profile[id]');
            //var_dump(Yii::$app->request->post());die();
            $root = Yii::getAlias('@webroot');
            $dir = $root.'/images/upload/'.$d[0].$d[1].$d[2];
            try{
                 @mkdir($dir);

            }finally{
                $IconName = $dir.'/'.$r.'.png';
                $icon = fopen($IconName,'wb');
                $base64Pic = substr(Yii::$app->request->post('Profile')['gravatar_email'],22);
                //echo $base64Pic;die();
                fwrite($icon, base64_decode($base64Pic));
                fclose($icon);
                $IconFile = '/images/upload/'.$d[0].$d[1].$d[2].'/'.$r.'.png';
            }

            //Yii::$app->request->post('Profile[gravatar_email]') = $IconName;
        }


        if ($model->load(Yii::$app->request->post())) {
            $model->gravatar_email = $IconFile;;
            if($model->save()){
